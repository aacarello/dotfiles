...
# Error en último comando ejecutado
ERROR_ANTERIOR=$?
...
function promptcmd () {
    ...
    # Error en el comando anterior
    if [ "$ERROR_ANTERIOR" != 0 ] ; then
    PS1="${PS1}\[${COLOR_LIGHTRED}\]E "
    else
    PS1="${PS1}"
    fi
  
        
    # Nro. de comando introducido en history
   ...
   # git-completion
    if [ -f ~/.git-completion.sh ]; then
        PS1="${PS1}\[${COLOR_BROWN}\]$(__git_ps1 ' (%s)')"
    fi
